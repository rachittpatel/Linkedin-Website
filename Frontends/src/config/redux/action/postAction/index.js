...
export const incrementPostLike = createAsyncThunk(
  ...
  async ({ post_id, user_id }, thunkAPI) => {
    try {
      const response = await clientServer.post("/increment_post_like", { post_id, user_id });

      // Optimistic update
      thunkAPI.dispatch(incrementPostLikeLocally({ post_id, user_id }));
...
export const getAllComments = createAsyncThunk(
  ...
  async ({ post_id }, thunkAPI) => {
    try {
      const response = await clientServer.post("/get_comments", { post_id });
      return thunkAPI.fulfillWithValue({
        comments: response.data.comments,
        postId: post_id,
      });
